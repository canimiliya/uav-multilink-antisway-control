...
def schema_columns(n_links=5):
    ...
    return base+joints+["ax_cmd_raw","ax_cmd_limited","ax_saturated","ax_slew_limited","anchor_active","roll_rad","pitch_rad","yaw_rad","thrust_cmd_raw_N","thrust_cmd_limited_N","mx_cmd_raw_Nm","my_cmd_raw_Nm","mz_cmd_raw_Nm","mx_cmd_limited_Nm","my_cmd_limited_Nm","mz_cmd_limited_Nm","rotor_motor_max_abs_cmd","solve_time_ms","controller","position_error_x","velocity_error_x","disturbance_hat","qp_status_code","qp_iterations","preview_horizon_steps","tip_weight","position_scale"]
...
def run_scene(model_config_path, da_config, scene, wind_path, reference_path, output_csv, temperature=None, duration_s=12.0):
    ...
    q=np.load(root/"artifacts/s4/lqr/Q.npy"); B=np.load(root/"artifacts/s4/linearization/B.npy"); A=np.load(root/"artifacts/s4/linearization/A.npy"); K=np.load(root/"artifacts/s4/lqr/K.npy")
