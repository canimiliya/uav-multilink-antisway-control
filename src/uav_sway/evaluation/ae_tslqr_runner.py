...
def run_scenario(candidate, scenario, wind_csv, reference_csv, output_csv, root, duration_s=12.0):
    ...
    for step in range(physics_steps + 1):
        ...
        if step % outer_steps == 0:
            ...
            tip_pos_now, tip_vel_now, axis_now, _ = task_map.kinematics_from_mujoco(data)
            task_ref_now = task_map.pose.tip_relative_position_m + np.asarray([external.x_ref, external.y_ref, external.z_ref])
            position_error_now = tip_pos_now - task_ref_now
            orientation_dot_now = float(np.clip(np.dot(axis_now, task_map.pose.cutter_axis_world), -1.0, 1.0))
            orientation_error_now = float(np.rad2deg(np.arccos(orientation_dot_now)))
            tip_speed_now = float(np.linalg.norm(tip_vel_now - np.asarray([external.vx_ref, 0.0, 0.0])))
            internal = controller.internal_reference(external); state = layout.extract(model, data, internal)
            started = wall_time.perf_counter_ns(); controller.command(
                state, external, measured_tip_error, 0.05,
                task_position_error_m=float(np.linalg.norm(position_error_now[[0, 2]])),
                task_orientation_error_deg=orientation_error_now,
                task_tip_speed_m_s=tip_speed_now,
            ); last_solve = (wall_time.perf_counter_ns() - started) / 1.0e6; outer_calls += 1
